...
def compute(program, inputs, addrOp, force):
...
def extract_tiles(tiles, type):
    specifics = []
    for tile in tiles:
        if tile[2] == type:
            specifics.append(tile)
    return specifics
...
def print_board(tiles):
    ...
    for tile in tiles:
        if tile[0] >= 0:
            printables[str(tile[0]) + "," + str(tile[1])] = tile[2]
        else:
            score = tile[2]
    for j in range(0, max([tile[1] for tile in tiles]) + 1):
        line = ""
        for i in range(0, max([tile[0] for tile in tiles]) + 1):
            type = printables[str(i) + "," + str(j)]
            if type == 3:
                line += "T"
            elif type == 4:
                line += "O"
...
def init(path_name, file_name):
    program = open_program(path_name, file_name)
    outputs, memory, _ = compute(program, [], 0, 0)
    tiles = [outputs[i:i+3] for i in range(0, len(outputs), 3)]
    blocks = extract_tiles(tiles, 2)
    return outputs, memory, len(blocks)


def solution(path_name, file_name):
    outputs, memory, blockCounts = init(path_name, file_name)
    print(blockCounts)
    tiles = [outputs[i:i+3] for i in range(0, len(outputs), 3)]
    score, decision = 0, []
    while blockCounts > 0:
        outputs, memory, _ = compute(
            program_from_memory(memory), decision, 0, 2)
        tiles = [outputs[i:i+3] for i in range(0, len(outputs), 3)]
        balls = extract_tiles(tiles, 4)
        paddles = extract_tiles(tiles, 3)
        blockCounts = len(extract_tiles(tiles, 2))
        if balls[-1][0] < paddles[-1][0]:
            decision = [-1]
        elif balls[-1][0] > paddles[-1][0]:
            decision = [1]
        else:
            decision = [0]
        score = print_board(tiles)
        time.sleep(0.05)
        os.system("clear")
