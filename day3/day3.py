...
def construct_wire(splitWire):
    ...
    for exec in splitWire:
        ...
        if dir == "R":
            ...
            coords += generate_coords(currentX - step, currentX, currentY, currentY + 1)[::-1]
            currentX = currentX - step
        elif dir == "U":
            coords += generate_coords(currentX, currentX + 1, currentY + 1, currentY + step + 1)
            currentY = currentY + step
        elif dir == "D":
            coords += generate_coords(currentX, currentX + 1, currentY - step, currentY)[::-1]
            currentY = currentY - step
    return coords
...
def closest(wire1, wire2):
    builded_wire1, builded_wire2 = construct_wire(wire1.split(",")), construct_wire(wire2.split(","))
